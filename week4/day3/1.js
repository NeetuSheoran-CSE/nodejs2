...
//npx autocannon http://localhost:8080/home -c 100 -d 10 -p 10
...
const routeLoggerMiddlewere = (req, res, next) => {
    const record = `Route name is: ${req.path} and method is: ${req.method}\n`;
    fs.appendFileSync("1.txt", record);
    next();
}
// app.use(routeLoggerMiddlewere); // use the route logger middleware
...
app.use(watchmanMiddlewere); // use the watchman middleware
